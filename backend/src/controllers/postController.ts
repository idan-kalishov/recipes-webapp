...
const addPost = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        await userModel.findByIdAndUpdate(owner, {$push: {post: savedPost._id}});
...
const getPostById = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const post = await Post.findById(postId).populate("comments").populate("owner", "userName")
            .populate("likes", "userName");
        ;
        if (!post) {
...
const getPostsBySender = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const posts = await Post.find({owner: sender})
            .populate("comments").populate("owner", "userName")
            .populate("likes", "userName");
        res.json(posts);
    } catch (error) {
